	...
	if ((psdioh->SD_BUS_STATUS & idle_level) == idle_level) {
		...
		if ((psdioh->SD_CMD_STATE & SDIOH_CMD_FSM_IDLE) && (psdioh->SD_DATA_STATE & SDIOH_DATA_FSM_IDLE)) {
			...
			if ((psdioh->SD_TRANSFER & SDIOH_SD_MODULE_FSM_IDLE)) {
				...
				RTK_LOGI(TAG, "SD card module state machine isn't in the idle state !\r\n");
				return HAL_BUSY;
			}
		} else {
			RTK_LOGI(TAG, "CMD or DATA state machine isn't in the idle state !!\r\n");
			return HAL_BUSY;
		}
	} else {
		RTK_LOGI(TAG, "CMD or DAT[3:0] pin isn't in the idle state !!\r\n");
		return HAL_BUSY;
	}
}
...
u32 SDIOH_CheckTxError(u16 *status)
{
	...
	if (psdioh->SD_TRANSFER & SDIOH_ERR_OCCUR) {
		temp = psdioh->SD_STATUS1;
		if (status != NULL) {
			*status = temp | (psdioh->SD_STATUS2 << 8);
		}
...
u32 SDIOH_Init(u8 BusBitMode)
{
	...
	while (1) {
		...
		DelayUs(1);
		if (i++ > 10000) {
			RTK_LOGE(TAG, "Wait SDIOH clock ready timeout !\r\n");
			return HAL_TIMEOUT;
		}
	}
