...
u32 SOCPS_SYSIrq_HP(void *Data)
{
	...
	if (tickless_debug) {
		RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "SOCPS_SYSIrq_HP %x\n", Rtemp);
		if (WakeEventFlag_HP == FALSE) {
			RTK_LOGS(NOTAG, RTK_LOG_ERROR, "Oops: SOCPS_SYSIrq_HP Error %x !!!!!\n", Rtemp);
		}
	}
	return 0;
}
...
void SOCPS_SleepPG(void)
{
	...
	if (nDeviceIdOffset != PMU_MAX) {
		...
		if (tickless_debug) {
			RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "DBG: Sleep blocked because Dev %x  busy\n", nDeviceIdOffset);
		}
		return;
	}

	RTK_LOGS(NOTAG, RTK_LOG_INFO, "HPPG\n");
	...
	ipc_msg_temp.msg = (u32)&sleep_param;
	...
	RTK_LOGS(NOTAG, RTK_LOG_INFO, "HPPW\n");
...
void SOCPS_WakeFromPG_HP(void)
{
	...
	/* SCB->SHP[8~11] */
	irq_set_priority(SysTick_IRQn, INT_PRI7);
	irq_set_priority(PendSV_IRQn, INT_PRI7);
...
void SOCPS_CPUReset(void)
{
	...
	if (nDeviceIdOffset != PMU_MAX) {
		pmu_exec_wakeup_hook_funs(nDeviceIdOffset);
		if (tickless_debug) {
			RTK_LOGE(TAG, "Oops: Sleep Fail %x !!!!!\n", nDeviceIdOffset);
		}
	...
	if (WakeEventFlag_HP != TRUE) {
		WakeEventFlag_HP = TRUE;
		if (tickless_debug) {
			RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "SOCPS_CPUReset reset \n");
		}
		...
		if (tickless_debug) {
			RTK_LOGS(NOTAG, RTK_LOG_DEBUG, "SOCPS_CPUReset wakeup \n");
		}
...
void SOCPS_SleepCG(void)
{
	...
	RTK_LOGS(NOTAG, RTK_LOG_INFO, "HPCG\n");
	...
	RTK_LOGS(NOTAG, RTK_LOG_INFO, "HPCW\n");
...
void SOCPS_AONTimer(u32 SDuration)
{
	...
	/* use 32K */
	Rtemp = HAL_READ32(SYSTEM_CTRL_BASE_LP, REG_AON_ISO_CTRL);
	Rtemp &= ~BIT_AUX_FUNC_CLK_SEL;
	HAL_WRITE32(SYSTEM_CTRL_BASE_LP, REG_AON_ISO_CTRL, Rtemp);
	...
	HAL_WRITE32(SYSTEM_CTRL_BASE_LP, REG_AON_WAKE_TIMER0, SDuration);
}