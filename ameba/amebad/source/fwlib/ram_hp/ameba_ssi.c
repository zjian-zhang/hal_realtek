...
static const char *const TAG = "SSI";
...
void SSI_SetRole(SPI_TypeDef *spi_dev, u32 role)
{
	u32 Temp;
	u32 SpixRoleSel = 0;
	/* SPI1 can be only configured as master role on AmebaD */
	assert_param(!((spi_dev == SPI1_DEV) && (role == SSI_SLAVE)));

	if (spi_dev == SPI0_DEV) {
		SpixRoleSel = BIT_HS_SPI0_ROLE_SELECT;
	} else if (spi_dev == SPI1_DEV) {
		SpixRoleSel = BIT_HS_SPI1_ROLE_SEL;
	}

	Temp = HAL_READ32(SYSTEM_CTRL_BASE, REG_HS_SPI_CTRL);
	/* for both SPI1 & SPI0: 0, slave; 1, master */
	Temp &= ~SpixRoleSel;
...
)
{
	...
	if (DataFrameSize > 8) {
		...
		if (((Length & 0x03) == 0) && (((u32)(pTxData) & 0x03) == 0)) {
			...
			RTK_LOGE(TAG, "SSI_TXGDMA_Init: Aligment Err: pTxData=0x%x,  Length=%d\n", pTxData, Length);
			return FALSE;
		}
	} else {
		/*  8~4 bits mode */
...
)
{
	...
	if (DataFrameSize > 8) {
		...
		if (((Length & 0x03) == 0) && (((u32)(pRxData) & 0x03) == 0)) {
			...
			RTK_LOGE(TAG, "SSI_RXGDMA_Init: Aligment Err: pTxData=0x%x,  Length=%d\n", pRxData, Length);
			return FALSE;
		}
	} else {
		/*  8~4 bits mode */
